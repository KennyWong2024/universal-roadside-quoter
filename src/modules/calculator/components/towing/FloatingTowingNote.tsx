...
interface FloatingTowingNoteProps {
    partnerName: string;
    planName: string;
    ps: string;
    sd: string;
...
export const FloatingTowingNote = ({
    partnerName, planName, ps, sd, maneuver, totalKm, tollsList, calculatedAmount
}: FloatingTowingNoteProps) => {
    const [copied, setCopied] = useState(false);

    const formattedTotal = calculatedAmount.toLocaleString('es-CR', { style: 'currency', currency: 'CRC' });
    const hasExcedente = calculatedAmount > 0;
            ...
            <button
                ...
                className={`w-full py-5 px-6 rounded-2xl font-extrabold text-lg shadow-lg transition-all flex items-center justify-center gap-3
                    ${copied
                        ...
                        : 'bg-blue-600 hover:bg-blue-700 text-white hover:shadow-xl hover:-translate-y-1 active:scale-95'}`}
            >
                {copied ? (
